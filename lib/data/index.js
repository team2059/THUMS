...
var sqlite3 = require('sqlite3').verbose(), // SQLite3 database https://github.com/mapbox/node-sqlite3
    sqlite_database = new sqlite3.Database('database.sqlite_database'), // Create or connect to the database
    getTables,
    createTable,
    getPostBySlug,
    getAllPosts,
...
getModulesById = function (obj, callback) {
    sqlite_database.all("SELECT * FROM modules WHERE id =" + obj.join(" OR id= ") + " ORDER by ord", callback);
};
...
addModulesAndPost = function (obj, id, callback) {
    sqlite_database.run("INSERT INTO modules (type,title,content,ord) VALUES (?,?,?,?)", obj.type, obj.title, obj.content, obj.ord, function (err) {
        if (!err) {
