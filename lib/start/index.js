    ...
    return new Promise(function (resolve, reject) {
        if (typeof obj === 'string') {
            if (obj.indexOf('@app') >= 0) {
                var new_path = path.join(file, obj.replace('@app', ''));
                if (path.extname(new_path) === '.json') {
                    promiseFunctions.readJSON(new_path).then(function (module_data) {
                        resolve(module_data);
                    }).catch(function (e) {
                        reject(e);
                    });
                }
                if (path.extname(new_path) === '.js') {
                    return resolve(processFiles.minifyJavaScript(new_path));
                }
                if (path.extname(new_path) === '.html') {
                    promiseFunctions.readFile(new_path).then(function (data) {
                        return resolve(data);
                    });
                } else {
                    promiseFunctions.readFile(new_path).then(function (module_data) {
                        resolve(module_data);
                    }).catch(function (e) {
                        reject(e);
                    });
