...
Section = React.createClass({displayName: 'Section',
    ...
    render: function () {
        ...
        if (layouts[this.props.data.type]) {
            component_type = layouts[this.props.data.type];
        } else {
            component_type = layouts.text;
        }
        if (typeof this.props.data.slug !== 'undefined') {
            component_type = component_type.link;
        } else {
            component_type = component_type.read;
        }
