...
load_apps = function (req, res, next) {
    ...
    if (!apps.get_modules(root)) {
        return next();
    }
    var app_modules = apps.get_modules(root).slice(0, apps.get_modules(root).length);
    function goThroughModules() {
        var current_app = app_modules.splice(0, 1)[0];
        console.log(current_app);
        current_app(req, res, function() {
            if (app_modules.slice(0).length === 0) {
                next();
            } else {
                goThroughModules();
            }
