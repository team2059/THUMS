...
module.exports = (thums) => {
    ...
    let count = (table, column) => {
                ...
                new Module()
                    ...
                    .then((module) => {
                        resolve(module.toJSON().map(function (mod) {
                            mod.action = 'read';
                            mod.content = JSON.parse(mod.content);
                            return mod;
                        }));
                    });
            });
        ...
        /**
         * Same function as loadPost, but uses an id instead of a slug.
                        ...
                        .then((query_results) => {
                            ...
                            if (!results) {
                                ...
                                results.forEach(result => {
                                    post_list.push(loadPostById(result.post));
                                });

                                // Wait for posts to load.
                                Promise.all(post_list)
                                .then((posts) => {
                                    posts.map(post => {
                                        if (post.slug === null) {

                                            // Consistent strings throughout
